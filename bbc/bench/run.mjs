...
const log = readFileSync(path.join(here, "beebasm.log"), "utf8");
const sym = {};
...
session.loadDisc(path.join(here, "bench.ssd"));
session.keyDown(16);
session.reset(true);
...
const marks = [];
let validated = null;   // read at bs_empty, before the benchmarks move the turtle
...
for (const [, label] of BENCH) {
    ...
    marks.push(cyc());
    if (marks.length === 1) validated = session.readMemory(sym.state, 4);
...
console.log(`move16q8 validation: 256 moves -> x=${gx} y=${gy} (model x=${vx} y=${vy}) ` +
    (ok ? "MATCH" : "*** MISMATCH ***"));
...
const CALLED = new Set(["bs_smul16", "bs_smul168", "bs_move16q8", "bs_move16q12",
                        "bs_const16", "bs_rlocal16", "bs_wstate16", "bs_op16",
                        "bs_copy144", "bs_copy32"]);
console.log("benchmark                          cycles     net of call");
