...
See the grid table below for how a cell maps onto that.
"""
import os
import re
import sys
...
# --------------------------------------------------------------------- grid
# A grid is fixed by two shifts: pixel x >> XSH is the column, scanline y >>
# YSH is the row.  Everything else about the geometry follows from them, so
# there is no way to name a combination the addressing cannot express.
#
# The one real constraint is that a cell stay CONTIGUOUS in screen memory.
# A cell is (1<<XSH) pixels wide -- that is (1<<XSH)/2 byte-columns, which sit
# 8 bytes apart -- by (1<<YSH) scanlines.  So the bytes are contiguous only if
# the cell is one byte-column wide, or spans a whole 8-scanline character row.
# 80x64 qualifies (1 column x 4 scanlines); 40x64 would not.
#
#                        XSH  YSH  NSIZE
GRIDS = {"40x32": (2, 3, 4),
         "80x64": (1, 2, 8)}
GRID = os.environ.get("NANOGRID", "80x64")
if GRID not in GRIDS:
    raise SystemExit(f"NANOGRID must be one of {sorted(GRIDS)}")
XSH, YSH, NSIZE = GRIDS[GRID]

GW, GH = 160 >> XSH, 256 >> YSH
CELLH = 1 << YSH                  # cell height in scanlines
RPC = 8 // CELLH                  # grid rows per 8-scanline character row
COLSTEP = 1 << (XSH + 2)          # screen bytes from one column to the next
CELLB = ((1 << XSH) // 2) * CELLH  # bytes in a cell
if RPC > 1 and CELLB != CELLH:
    raise SystemExit(f"{GRID}: cell is not contiguous in screen memory")


def rowbase(r):
    """Screen address of grid row `r`, column 0."""
    return SCREEN + (r // RPC) * 640 + (r % RPC) * CELLH
...
class Compiler:
    ...
    def tables(self):
        ...
        e()
        e(f"; ---- screen row bases: &3000 + (row/{RPC})*640 + (row%{RPC})*{CELLH}")
        e(".rowlo")
        e("    EQUB " + ",".join(str(rowbase(r) & 0xFF) for r in range(GH)))
        e(".rowhi")
        e("    EQUB " + ",".join(str(rowbase(r) >> 8) for r in range(GH)))
        ...
        e(".spancnt")
        e("    EQUB " + ",".join(str(2 * s + 1) for s in range(NSIZE)))
        e(".spandy")
...
def main():
    ...
    with open(out, "w") as f:
        ...
        f.write(f"; grid {GRID}: a cell is {1 << XSH}px x {CELLH} rows "
                f"= {CELLB} contiguous bytes, columns {COLSTEP} apart\n")
        f.write(f"GW = {GW}\nGH = {GH}\nXSH = {XSH}\nYSH = {YSH}\n")
        f.write(f"CELLB = {CELLB}\nCOLSTEP = {COLSTEP}\nNSIZE = {NSIZE}\n")
        slo, shi, srate = p.spin if p.spin else (0, 0, 0)
        f.write(f"SPINLO = {slo}\nSPINHI = {shi}\nSPINRATE = {srate}\n")
        f.write(runtime.replace("; @@PROCS@@", "\n".join(body))
                       .replace("; @@TABLES@@", "\n".join(tables))
                       .replace("; @@ARRAYS@@", "\n".join(arrays)))
    print(f"{src}: grid {GRID}, {len(p.procs)} procs, "
          f"{len(c.dists)} move distances, {len(p.plan)} tints")
